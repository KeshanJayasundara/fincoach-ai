    ...
    <section id="features" className="py-14 sm:py-20 px-4 sm:px-5 bg-gradient-to-b from-transparent to-[#0F0D22]">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 reveal">
          {[
            { icon: "🤖", title: "AI Finance Coach", desc: "Ask anything about your money. Get personalized answers based on your actual spending data and profession." },
            { icon: "👥", title: "Multi-Role Support", desc: "Doctor by day, freelancer by night? Track finances with intelligent role-tagging across all your income sources." },
            { icon: "🌍", title: "Multi-Currency", desc: "LKR, USD, EUR, GBP and 28 more. All amounts auto-converted to your base currency in real time." },
            { icon: "📊", title: "Smart Dashboard", desc: "Visual spending breakdowns, trend charts, and goal progress — all filterable by role or time period." },
            { icon: "📧", title: "Monthly Reports", desc: "One combined report auto-sent on the last day of every month. Request on-demand anytime for any past month." },
            { icon: "🧾", title: "Bill Scanning", desc: "Scan a bill image and AI extracts amount, category, and date automatically. Zero manual entry needed." },
          ].map(({ icon, title, desc }) => (
            <div key={title} className="glow-card p-5 sm:p-6 rounded-2xl bg-white/3 border border-white/7 transition-all duration-300 hover:bg-white/5 group">
              <div className="feat-icon w-10 sm:w-11 h-10 sm:h-11 rounded-xl flex items-center justify-center text-lg sm:text-xl mb-3 sm:mb-4 group-hover:scale-110 transition-transform">{icon}</div>
